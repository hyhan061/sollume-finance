...
_DEFAULTS = {
    # 2026-07-23 hoyeon.han: 정산서 유형(매출/매입) — 매입 지원 추가
    "sseller_doc_type": ss.TYPE_SALE,
    "sseller_file_sig": None,  # (file_path, sheet_name, mtime, doc_type)
...
render_page_header(
    "정산서 생성 [셀러]",
    # 2026-07-23 hoyeon.han: 매입 정산서 지원 추가로 "추후 지원 예정" 문구 제거
    # "서버에 저장된 발주내역 하나로 일자·거래처·셀러(비고)별 정산서를 생성합니다. "
    # "(매출 기준 — 매입 정산서는 추후 지원 예정)",
    "서버에 저장된 발주내역 하나로 일자·거래처·셀러(비고)별 "
    "매출/매입 정산서를 생성합니다.",
    icon="🧾",
)
...
# 2026-07-23 hoyeon.han: 매입 정산서 지원 추가. 유형에 따라 컬럼 매핑(column_map)과
# 셀러 시트 제목(title_const)을 전환한다 (pages/settlement.py 의 매출/매입 라디오 패턴).
st.markdown("### 🧭 정산서 유형")
doc_type = st.radio(
    "정산서 유형",
    options=[ss.TYPE_SALE, ss.TYPE_PURCHASE],
    horizontal=True,
    key="sseller_doc_type_radio",
    index=0 if st.session_state.sseller_doc_type == ss.TYPE_SALE else 1,
    help="매출 기준(상품매출)과 매입 기준(상품매입) 정산서를 전환합니다.",
)
st.session_state.sseller_doc_type = doc_type
# 유형별 컬럼 매핑 / 셀러 시트 제목 — 이후 모든 ss.* 호출에 이 column_map 을 넘긴다.
...
if prev_sheet in sheet_names:
    ...
    default_index = 0
# 2026-07-09 hoyeon.han: 디자인 개선 - 셀렉트 폭을 측정폭으로 제한 + 남는 가로를 도움말 레일로 사용
...
# 2026-07-23 hoyeon.han: 유형(doc_type)도 시그니처에 포함 → 매출↔매입 전환 시
# file_sig 가 달라져 기존 _reset_below_file() 이 발동(시트 선택은 유지, 일자·
# 거래처·셀러·plan 초기화). 로드 컬럼 집합도 유형에 따라 달라지므로 캐시도 갱신됨.
...
# 2026-07-23 hoyeon.han: 매입 지원 - order_doc_type 인자 추가(캐시 키 분리) + 유형별 맵 로드
def _load_orders(
    ...
    order_doc_type(매출/매입)에 따라 로드할 컬럼 집합이 달라 캐시 키에 포함한다.
...
try:
    # 2026-07-23 hoyeon.han: 매입 지원 - doc_type 전달
    # df_orders = _load_orders(file_path, selected_sheet, mtime)
    df_orders = _load_orders(file_path, selected_sheet, mtime, doc_type)
...
# 2026-07-23 hoyeon.han: 매입 지원 - 이하 모든 ss.* 호출에 유형별 column_map 전달
date_range = ss.extract_date_range(df_orders, column_map=column_map)
# 2026-07-09 hoyeon.han: 디자인 개선 - '데이터 기간'은 맥락이라 metric에서 잘림('…') 유발 → 캡션으로, metric은 숫자만
...
# 2026-07-09 hoyeon.han: 디자인 개선 - 셀렉트 측정폭 제한 + 남는 가로를 도움말 레일로
...
# 2026-07-23 hoyeon.han: 매입 지원 - 하드코딩된 SALE 맵 대신 선택된 유형 맵 사용
...
df_seller_rows = ss.filter_by_sellers(df_pv, selected_sellers, column_map=column_map)
...
if st.button("🔍 미리보기 생성", key="sseller_preview_btn"):
    try:
        st.session_state.sseller_plan = ss.build_sheet_plan(
            ...
            column_map=column_map,  # 2026-07-23 hoyeon.han: 매입 지원
        )
...
if plan is not None:
    ...
    # 2026-07-06 hoyeon.han: 파일명은 시트 1개면 '정산서_{유형}_{업체명}_{시트명}',
    # 여러 시트면 '정산서_{유형}_{업체명}' (build_output_filename 참고)
    # 2026-07-23 hoyeon.han: 매입 지원 - doc_type 전달(유형이 파일명에 반영됨)
    out_filename = ss.build_output_filename(
        selected_vendor, list(plan["seller_sheets"].keys()), doc_type=doc_type
    )
    ...
    if st.button("📥 정산서 생성", key="sseller_generate_btn", type="primary"):
        try:
            ...
            out_filename = ss.build_output_filename(
                ...
                doc_type=doc_type,  # 2026-07-23 hoyeon.han: 매입 지원
            )
            out_path = os.path.join("processed", out_filename)
            period_line = ss.build_period_line(selected_vendor, **date_kwargs)
            ss.write_seller_settlement_xlsx(
                out_path=out_path,
                # 2026-07-23 hoyeon.han: 매입 지원 - 유형별 시트 제목/컬럼 매핑 사용
                # title=ss.TITLE_SALE,
                title=title_const,
