...
while date_from <= date_to:
    ...
    df_sales_today = df[ (df['출고일'] == today) & (df['업체명'] == sale) & (df['계산서'] == '대상') & ((df['상품매출'] != 0) | (df['판매배송비'] != 0) | (df['도선료'] != 0)) ]
    ...
    df_buy_today = df[ (df['출고일'] == today) & (df['업체명'] == sale) & (df['매입처'] != '당사재고') & (df['매입처'] != '솔루미랩') & ((df['상품매입'] != 0) | (df['매입배송비'] != 0) | (df['도선료.1'] != 0)) ]
