...
fileName = sys.argv[1]
#fileName = "솔루미랩_발주내역_240925.xlsm"
_day = sys.argv[2]
#_day = "2024-09-24"
...
#################### 매입 데이터 조회 ####################
excelWriter = pd.ExcelWriter('솔루미랩_일괄등록_매입_' + _day + '.xlsx', engine='xlsxwriter')   # 매입 엑셀파일 별도 생성
...
for row in df_10.itertuples():
    ...
    sheetName = re.sub(r"\s*\[.*?\]", '', row.매입처 + '_' + row.과세구분 + '_' + ('empty' if row.특이사항 is None else row.특이사항) )     # 시트명에 특수문자 제거
