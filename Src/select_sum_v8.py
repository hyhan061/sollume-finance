...
v8 : 단가 추가
2025-09-24 : 수량 != 0 인것만 필터링 하도록 수정
...
#fileName = '솔루미랩_발주내역_20251020.xlsm'
#_from = '2025-10-01'
#_to = '2025-10-20'
...
while date_from <= date_to:
    ...
    df_sales_today = df[ (df['출고일'] == date_from) & (df['계산서'] == '대상') & (df['수량'] != 0) & ((df['상품매출'] != 0) | (df['판매배송비'] != 0) | (df['도선료'] != 0)) ].copy()

    # 특이사항 NaN을 공란으로 수정
    df_sales_today.loc[:, '특이사항'] = df_sales_today['특이사항'].fillna('')
    df_sales_today.loc[:, '상품매출'] = df_sales_today['상품매출'].infer_objects(copy=False).fillna(0)
    ...
    # 상품매출 소수점 삭제
    df_sales_today.loc[:, '상품매출'] = df_sales_today['상품매출'].apply(np.floor)
    # 숫자로 변환
    df_sales_today['상품매출'] = to_num(df_sales_today['상품매출'])
    df_sales_today['수량'] = to_num(df_sales_today['수량'])

    # 로그
    #print(df_sales_today['수량'].apply(type).value_counts())
    # 단가 구하기
    #df_sales_today['단가'] = df_sales_today.apply(lambda x : x['상품매출'] / x['수량'], axis=1)
    df_sales_today.loc[:, '단가'] = df_sales_today['상품매출'] / df_sales_today['수량']
    ...
    if len(df_sales_today) > 0:
        ...
        df_sales_today.loc[:, ['출고일', '업체명', '특이사항', '제품', '수량', '상품매출', '단가', '판매배송비', '도선료', '업체별합계', '과세구분', '계산서', '구분']].to_excel(excelWriter, sheet_name='매출_' + today)
        # 일자별 매출 데이터 추가
        new_data = pd.DataFrame({'매출일자': [today], '합계금액': [sum_sales]})
        #frames = [df_sales_today, new_data];    # dataframe 2개를 배열로
        #frames = [f for f in frames if not f.empty and not f.isna().all(axis=None)];  # 비어있거나, null인 dataframe은 제외한다.

        if df_sales_daily.empty:
            df_sales_daily = new_data.copy()    # 첫번째 데이터이면 new_data로 대체
        else:
            df_sales_daily = pd.concat([df_sales_daily, new_data], ignore_index=True)   # 이미 있으면 누적함
        #df_sales_daily = pd.concat(frames, ignore_index=True)   # apply frames filtered
    ...
    df_buy_today = df[ (df['출고일'] == date_from) & (df['특이사항'] != '솔루미재고') & (df['매입처'] != '당사재고') & (df['매입처'] != '솔루미랩') & (df['수량'] != 0) & ((df['상품매입'] != 0) | (df['매입배송비'] != 0) | (df['도선료.1'] != 0)) ].copy()
    ...
    # 로그
    #print(df_buy_today['상품매입'].apply(type).value_counts())
    # 단가 구하기
    df_buy_today.loc[:, '단가'] = df_buy_today['상품매입'] / df_buy_today['수량']
    ...
    # '매입처' 유라이크 도선료 0처리. 2024-11-04 추가
    df_buy_today.loc[df_buy_today['매입처'] == '유라이크', '도선료.1'] = 0
    ...
    if len(df_buy_today) > 0:
        ...
        df_buy_today.loc[:, ['출고일', '매입처', '특이사항', '제품', '수량', '상품매입', '단가', '매입배송비', '도선료.1', '업체별합계', '과세구분', '구분', '업체명']].to_excel(excelWriter, sheet_name='매입_' + today)
        new_data = pd.DataFrame({'매입일자': [today], '합계금액': [sum_buy]})

        # frames = [df_buy_today, new_data];  # dataframe 2개를 배열로 설정
        # frames = [f for f in frames if not f.empty and not f.isna().all(axis=None)];    # 비어있어나, NaN인 dataframe은 필터링한다.

        if df_buy_daily.empty:
            df_buy_daily = new_data.copy()
        else:
            df_buy_daily = pd.concat([df_buy_daily, new_data], ignore_index=True)
        # df_buy_daily = pd.concat(frames, ignore_index=True) # apply frames filtered
...
df_sales_daily.to_excel(excelWriter, sheet_name='매출_일자별합계')
df_buy_daily.to_excel(excelWriter, sheet_name='매입_일자별합계')
