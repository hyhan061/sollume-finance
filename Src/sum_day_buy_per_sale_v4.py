"""
v3 : 2025년 발주내역으로 수정 TODO : 발주내역 시트명을 인수로 받는다?
v4 : 일자별 매출/매입 합계 시트 추가
"""

"""
특정날짜 범위 매출처별 매입금액 확인
"""
...
# 엑셀 writer (파일 이름에 매출처 추가)
excelWriter = pd.ExcelWriter(f'솔루미랩_{sale}_{_from}_{_to}.xlsx', engine='xlsxwriter')
...
while date_from <= date_to:
    ...
    df_sales_today = df[ (df['출고일'] == date_from) & (df['업체명'] == sale) & (df['계산서'] == '대상') & ((df['상품매출'] != 0) | (df['판매배송비'] != 0) | (df['도선료'] != 0)) ].copy()
    ...
    if len(df_sales_today) > 0:
        ...
        if df_sales_daily.empty:
            df_sales_daily = new_data.copy()
        else:
            df_sales_daily = pd.concat([df_sales_daily, new_data], ignore_index=True)

    #################### 매입 데이터 조회 ####################
    df_buy_today = df[ (df['출고일'] == date_from) & (df['업체명'] == sale) & (df['매입처'] != '당사재고') & (df['매입처'] != '솔루미랩') & ((df['상품매입'] != 0) | (df['매입배송비'] != 0) | (df['도선료.1'] != 0)) ].copy()
    ...
    df_buy_today.loc[:, '도선료.1'] = df_buy_today['도선료.1'].fillna(0)
    ...
    if len(df_buy_today) > 0:
        df_buy_today.loc[:, ['출고일', '매입처', '특이사항', '제품', '수량', '상품매입', '매입배송비', '도선료.1', '업체별합계', '과세구분', '구분']].to_excel(excelWriter, sheet_name='매입_' + today)
